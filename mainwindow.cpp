...
{
    QMessageBox::information(this,"Button clocked","buttons");
    QTableWidget *table = ui->tableWidget;
    QLineEdit *calEdit = ui->lineEditCalculate;
    ...
    // Display the entered expression and result column name
//    QLabel *labelExpression = ui->labelExpression;
//    labelExpression->setText("Expression: " + expression + " | Result Column: " + resultColumnName);
...
void MainWindow::calculateResult(const QString &expression)
{
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        ...
        // Identify the result column header
        int headerIndex = -1;
        for (int i = 0; i < columnHeaders.count(); ++i) {
            if (columnHeaders[i] == resultColumnName) {
                headerIndex = i;
                break;
            }
        }
        ...
        QTableWidgetItem *resultItem = table->item(row, headerIndex);
        if (!resultItem) {
            resultItem = new QTableWidgetItem();
            table->setItem(row, headerIndex, resultItem);
        }
...
QStringList MainWindow::parseExpression(const QString &expression)
{
    ...
    columnHeaders = expression.split(regex, Qt::SkipEmptyParts);
...
int MainWindow::findOrCreateResultColumn(const QStringList &columnHeaders)
{
    ...
    for (int col = 0; col < table->columnCount(); ++col) {
        if (table->horizontalHeaderItem(col)->text() == resultColumnName) {
...
double MainWindow::evaluateExpressionForRow(const QString &expression, int row)
{
    ...
    // Implement your own logic to evaluate the expression for each row
    // For simplicity, let's assume the expression format is like "header1 + header2"
    QStringList columnHeaders = parseExpression(expression);
    double result = 0.0;

    // Create a map between headers and their corresponding table columns
    QMap<QString, int> headerIndexMap;
    for (int i = 0; i < columnHeaders.count(); ++i) {
        for (int col = 0; col < table->columnCount(); ++col) {
            if (table->horizontalHeaderItem(col)->text() == columnHeaders[i]) {
                headerIndexMap[columnHeaders[i]] = col;
                break;
            }
        }
    }

    // Evaluate the expression for the specific row
    for (const QString &header : columnHeaders) {
        if (headerIndexMap.contains(header)) {
            QTableWidgetItem *item = table->item(row, headerIndexMap[header]);
            if (item) {
                bool isNumeric;
...
void MainWindow::on_clear_triggered()
{
    ...
    // Iterate through the table and clear the QTableWidgetItem objects in each cell
    for (int row = 0; row < table->rowCount(); ++row) {
        for (int col = 0; col < table->columnCount(); ++col) {
            QTableWidgetItem *item = table->item(row, col);
            if (item) {
